...
MOBILE_FACE_NET_WEIGHTS_PATH = './pretrained/mobilefacenet_model_best.pth.tar'
...
from Models.StyleGan2.model import model_style
# from Models.StyleGan2.model import stylise
...
if config['use_pre'] == True:
    attr_encoder = torch.load('./pretrained/Models/attr_long.pt')
    attr_encoder = Inception(attr_encoder) 
    mlp = torch.load('./pretrained/Models/maper_long.pt')
    discriminator = torch.load('./pretrained/Models/dis_long.pt')
else : 
...
run = wandb.init(project="ID_disantalgement_style_transfer", reinit=True, config=config, name=run_name)
...
test_id_images = model_style(test_id_images)
test_attr_images = model_style(test_attr_images)
...
with tqdm(total=config['epochs'] * len(train_loader)) as pbar:
    for epoch in range(config['epochs']):
        ...
        for idx, data in enumerate(train_loader):
            ...
            id_images = model_style(id_images)
            attr_images = model_style(attr_images)
            ...
            if use_descrimantive_loss and idx % 2 == 0:
                ...
                wandb.log({'G_Error': g_error.detach().cpu()}, step=Global_Config.step)
                wandb.log({'D_Error': ((error_real+error_fake)/2).detach().cpu()}, step=Global_Config.step)
                
                
            else:
                
                total_error = trainer.non_adversarial_train_step(
                    id_images, attr_images, fake_data, real_landmarks, use_rec_extra_term,id_vec)
                ...
                with torch.no_grad():
                    ...
                    with torch.no_grad():
                        for idx in range(len(test_ws)):
                            ...
                            wandb.log(
                                {f"ID_Image{idx}": [
                                    wandb.Image(test_id_images[idx] * 255, caption=f"Train_ID_Image{idx}")]},
                                step=Global_Config.step)
                            wandb.log(
                                {f"Attribute_Image{idx}": [
                                    wandb.Image(test_attr_images_cycled[idx] * 255, caption=f"Train_ID_Image{idx}")]},
                                step=Global_Config.step)
                            wandb.log(
                                {f"Generated{idx}": [
            ...
            if (idx % 8000) == 0:
                torch.save(mlp, f'{MODELS_DIR}maper_long{time.time()}_{int(12)}.pt')
                torch.save(attr_encoder, f'{MODELS_DIR}attr_long{time.time()}_{int(12)}.pt')
                torch.save(discriminator, f'{MODELS_DIR}dis_long{time.time()}_{int(12)}.pt')
